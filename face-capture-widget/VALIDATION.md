...
组件已统一使用 `verificationAction`，支持每轮一种独立动作。31 项组件测试、15 项 SDK 测试、16 项演示页测试与 145 项 .NET 测试通过；类型检查、构建和 npm pack 通过。组件安装包需与支持新动作接口的 Agent 配套使用。

已用合成帧服务验证随机动作提示、重拍、取消、照片返回及桌面/移动布局。Windows 安装包于 2026-09-18 构建成功，未执行安装升级或真人张嘴/转头验收。完整范围见[交付记录](../docs/action-verification-validation.md)。

以下内容为此前组件版本的历史验证记录，测试数量和“未修改 Agent/SDK”等描述不适用于本次更新。
